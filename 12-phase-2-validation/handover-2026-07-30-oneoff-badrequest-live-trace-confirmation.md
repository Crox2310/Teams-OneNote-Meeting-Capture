...
## ⏭ SUPERSEDED — see `2026-07-31-oneoff-design-evidence-meetingid-column.md`

**This doc's investigation is complete and its findings are correct, but its "Fourth addendum" open design question has since been answered.** As of 31 July, a full finalised, buildable design exists in `12-phase-2-validation/2026-07-31-oneoff-design-evidence-meetingid-column.md` — including the exact root cause refinement (`Condition_Should_Create_Page` reads a Compose action that never runs on the one-off path), the chosen fix architecture (Option A — shared variables), and a 9-step numbered build plan with copy-paste-ready expressions.

**Do not start a build session from this doc.** Go to the 31 July doc's own "START HERE NEXT SESSION" block instead — it is the current source of truth for what to build next.

This doc is retained below as the original evidence trail (30 July live-trace investigation) that the 31 July design builds on — still useful for understanding *how* the root cause was found, just no longer the place to start.
...
**Open design question for the fix — RESOLVED on 31 July, see `2026-07-31-oneoff-design-evidence-meetingid-column.md`.** (Original text retained for historical record: how *should* the flow find "the existing page for this one-off meeting" on recapture, given there's no SeriesMasterId-based mapping to rely on? Candidate approaches evaluated: match by meeting title within the resolved section; or a new one-off-specific mapping mechanism. The 31 July doc found and confirmed the actual answer: reuse the existing, already-partially-wired `MeetingId` column in the SharePoint mapping list, keyed on the Outlook calendar event ID Flow B's own trigger already calls `MeetingId`. It also found something this doc didn't catch: the top-level create-vs-reuse decision (`Condition_Should_Create_Page`) itself never receives a valid input for one-off meetings, which is a more complete explanation than the "missing lookup" framing below.)

## Status

- **Root cause:** **CONFIRMED, and revised in scope** (30 July), **then further refined and finalised into a buildable design (31 July — see the 31 July doc).** Immediate technical cause: `Set varOutputPageSelfUrl Existing` is missing its `value` key (Pattern 6), so `varOutputPageSelfUrl` = `""` → `Compose ExistingPageId` = `""` → empty `pageId` → `UpdatePageContent` `BadRequest` (misleadingly reported against `sectionId`). **Underlying cause (as understood 30 July):** there is no mechanism in this flow that resolves a specific existing OneNote page for a **one-off** meeting on recapture. **Underlying cause (fully refined 31 July):** additionally, `Condition_Should_Create_Page` itself reads an action output that only exists on the recurring path, so the create-vs-reuse decision is undefined — not just missing a lookup — for every one-off meeting.
...
- **Fix:** Fully designed as of 31 July, not yet implemented. See `2026-07-31-oneoff-design-evidence-meetingid-column.md` for the 9-step build plan.
- **Process note:** `amendment-log.md` backfill is still outstanding per the 20 July gap-analysis doc — should cover the Pattern 6 finding here, plus the eventual fix once built.
