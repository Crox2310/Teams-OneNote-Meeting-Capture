# Process Note: Expression Audit — Maintenance Instructions for Future Sessions

## What this document is

This is a standing process instruction, not a dated session handover. It should be read by Claude at the start of any session involving `expression-audit.md` (or whatever the audit document is ultimately named) in this repo, alongside the most recent dated handover note.
...
The audit's purpose is to catch all remaining instances of these three patterns in one pass, rather than continuing to discover them one at a time through live debugging — which is expensive in both time and live-system risk (see the connection-incident handover from 2026-06-20 evening).

## The core instruction for future sessions

**The audit document is a snapshot, not a live sync.** It is only accurate as of the last time it was explicitly updated. Any session in which an action's expression is changed in Designer — whether as a bug fix, a deliberate enhancement, or any other edit — MUST end with the audit document being updated to reflect the new expression, before the session's handover note is finalized.

This applies even to small or seemingly trivial edits. The whole value of the audit doc is that it can be trusted as accurate; a single un-logged drift makes every future read of it less reliable.

## What "updating the audit" means in practice

When an expression changes during a session:
1. Identify the action's entry in the audit document (organized by flow, then by action name).
...
Without this discipline, the audit document will silently go stale within a session or two, at which point it becomes actively misleading rather than just incomplete — worse than not having it at all, since future sessions (including different Claude instances, given memory does not persist between conversations) would trust outdated information. The dated handover notes capture *what happened in a session*; the audit document is meant to capture *current ground truth*, and only stays useful if it's treated as such.

## Pointer for future Claude instances

If you are picking up this project fresh: read this file first, then the most recent dated handover note, then `expression-audit.md` itself. If `expression-audit.md` doesn't exist yet, the audit is still in progress — check the most recent handover for where it was left off.
