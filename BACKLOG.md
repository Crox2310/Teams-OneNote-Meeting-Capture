...
**Last updated:** 25 Sep 2026 — BL-38 built and closed; BL-35/37 added this session.
...
| ID | Item | Closed | How |
|---|---|---|---|
| BL-38 | Flow A candidate list 3-level capture status indicator | 25 Sep 2026 | Built and confirmed working. SP: `RecapCaptured` (Yes/No) column. Flow C: `FC05u_Set_RecapCaptured` PatchItem after FC05t inside FC05c True branch. Flow A: `FA-GetMappingRows` Get items + `FA36a_Filter_MappingMatch` (Query) + `FA36b_Compose_StatusLabel` + FA36 updated. Labels bold in Agent chat (`**Captured**` / `**Mtg Notes**`). FA33A/FA34A wiped twice during session — values restored from reference. |
