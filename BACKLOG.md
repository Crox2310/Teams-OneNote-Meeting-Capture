...
**Last updated:** 25 Sep 2026 — BL-38 simplified to a 3-state (recap-only) design; BL-35/37/38 added/reworked this session.
...
| BL-22 | **Recap duplicated when chat step fails.** FC16 (`Update_Item_ChatCaptured`) currently runs after FC15: Succeeded only. But FC05t has already appended the recap — so if FC15 fails, the next retry re-appends the recap with no chat content. **Q6 decision: `ChatCaptured` should only be true when content was actually written.** FC16 must require both FC05t (recap) and FC15 (chat) to have Succeeded. Accept duplicate recaps on retry rather than marking captured with missing content. | Bug | H | R-02 / Q6 answer | Settings. FC16 run-after: require FC05t Succeeded AND FC15 Succeeded. **Note (25 Sep):** BL-38 was simplified to a recap-only display flag and no longer touches the chat branch, so it does NOT resolve this race. This item stands on its own. |
...
| BL-10 | **No re-fill after a late recap.** Once `ChatCaptured` is true the occurrence is never revisited. | Improve | L | Architecture §9 | Options: separate `RecapCaptured` flag, or second pass at EndTime + N hours. **Note (25 Sep):** BL-38 was simplified to a recap-only display flag with no Flow D change, so it does NOT resolve this. This item stands on its own. |
...
| BL-38 | **Flow A candidate list should show a 3-level capture status per meeting.** Blank (nothing captured) / Captured (page created, no recap yet) / Mtg Notes (recap added). Chat-transcript status intentionally NOT tracked (data low quality — David's call 25 Sep). Visible in the Agent chat meeting picker so David needn't open Copilot Studio or OneNote. | Improve | M | Chat 25 Sep | **Simplified design (25 Sep) — tracks the recap only, so it avoids the Flow C chat-branch rework and Flow D changes the earlier 5-state version needed.**<br>**3-state derivation (in Flow A):** no matching row -> Blank; row exists + `RecapCaptured` != true -> Captured; row exists + `RecapCaptured` = true -> Mtg Notes. Row-exists alone means Flow B created a page, so `SectionPagesUrl` is not needed as a separate signal.<br>**Change 1 - SharePoint:** add `RecapCaptured` (Yes/No, default No) column on RecurringMeetingSectionMap.<br>**Change 2 - Flow C:** add ONE PatchItem inside the FC05c condition, right after FC05t (the recap append), setting `RecapCaptured=true`. This is the only Flow C change. Display flag only — if it occasionally doesn't fire it degrades gracefully to showing "Captured" instead of "Mtg Notes". Do NOT touch the chat branch, FC16, or Flow D; Flow D keeps stopping at `ChatCaptured` as now; existing rows need no backfill.<br>**Change 3 - Flow A:** add ONE `Get items` on RecurringMeetingSectionMap filtered to the browsed date (`varDateContext`) before FA35; then FA36's expression derives one of the three labels via filter()/first() keyed on candidate `seriesMasterId` (recurring) or `id`=MeetingId (one-off). FA32 (single-match) returns an empty list so there is no line to annotate there; selection-mode FA15-FA26 is dead code — do not touch either. Renders as plain text in Agent chat, e.g. `1. Supply Chain Product Team Meeting - Mtg Notes` (no label shown for Blank).<br>**Estimate: ~2-2.5h, low risk — Sonnet-level.** The Opus-worthy ambiguity is gone. Pre-build check that still stands: re-pull live Peek Code for FC01 and the FC05c/FC05t area, since the reference snapshot (19 Sep) pre-dates the 21 Sep one-off changes (FC01 one-off branch, text_3/text_4).<br>**Note:** meetings that never get a recap (no Copilot licence, restricted invites e.g. row 442) sit at "Captured" permanently — that is a valid terminal state here, not an error. |
