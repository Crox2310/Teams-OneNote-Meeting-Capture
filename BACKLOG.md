...
| BL-22 | **Recap duplicated when chat step fails.** FC16 (`Update_Item_ChatCaptured`) currently runs after FC15: Succeeded only. But FC05t has already appended the recap — so if FC15 fails, the next retry re-appends the recap with no chat content. **Q6 decision: `ChatCaptured` should only be true when content was actually written.** FC16 must require both FC05t (recap) and FC15 (chat) to have Succeeded. Accept duplicate recaps on retry rather than marking captured with missing content. | Bug | H | R-02 / Q6 answer | Settings. FC16 run-after: require FC05t Succeeded AND FC15 Succeeded. **Note (25 Sep):** BL-38's design (split RecapCaptured/ChatCaptured flags + per-branch guards) resolves this race a different way — likely closes this item once BL-38 ships and is confirmed. Don't work both independently. |
...
| BL-10 | **No re-fill after a late recap.** Once `ChatCaptured` is true the occurrence is never revisited. | Improve | L | Architecture §9 | Options: separate `RecapCaptured` flag, or second pass at EndTime + N hours. **Note (25 Sep):** BL-38's design does exactly this (split flag + Flow D polls until both true) — likely closes this item as a side effect once BL-38 ships. Don't work both independently. |
...
| BL-38 | **Flow A candidate list should show a 5-level capture status per meeting.** Blank (nothing captured) / Captured (OneNote page created, no content) / Mtg Notes (recap added) / Chat Transcript (chat added) / Fully Captured (both added — label TBD, confirm wording). David wants this visible in Agent chat without checking Copilot Studio or OneNote directly. | Improve | M | Chat 25 Sep | **Design (checked 25 Sep — spans 3 flows, not just Flow A):**<br>1. SharePoint: add `RecapCaptured` (Yes/No, default No) column, separate from existing `ChatCaptured`.<br>2. Flow C: `FC05u` (end of recap branch) writes `RecapCaptured` only; `FC22` (end of chat branch) writes `ChatCaptured` only — currently both write the same `ChatCaptured` field (confirmed in ARCHITECTURE.md Section 3.5).<br>3. Flow C: add an "already done" guard to each branch's entry (skip recap branch if `RecapCaptured` already true; skip chat branch if `ChatCaptured` already true) — needed because Flow D will now keep polling a partially-captured row, and OneNote appends aren't idempotent, so the already-successful branch must not re-run.<br>4. Flow D: `FD04` stop condition changes from `ChatCaptured ne true` to `not(RecapCaptured eq true and ChatCaptured eq true)` — keep polling until both are done. This also resolves BL-10 (no re-fill after a late recap) as a side effect.<br>5. Flow A: `Get items` on `RecurringMeetingSectionMap` filtered to the browsed date; match candidates by SeriesMasterId (recurring) or CalendarEventId=MeetingId (one-off); derive the 5-state label from `SectionPagesUrl` (page exists — same field BL-07's guard already treats as the "row complete" signal) + `RecapCaptured` + `ChatCaptured`; insert into `FA32_Compose_OutCandidateList*` (single/multi-match, one-off branches).<br>**Interactions:** point 3 makes BL-22's original shared-flag race moot — close BL-22 once this ships and is confirmed working. Point 4 resolves BL-10 as a side effect — close BL-10 too once confirmed.<br>**Revised estimate: ~4-5 hours** (was ~2-2.5h before the Flow C/D implications were found) — still Sonnet-level, no Opus design needed, but now touches 3 flows so budget for structural-edit corruption risk at each. Do in its own session: schema then Flow C (verify no regression on existing capture before moving on) then Flow D then Flow A last (lowest risk, purely additive). |
